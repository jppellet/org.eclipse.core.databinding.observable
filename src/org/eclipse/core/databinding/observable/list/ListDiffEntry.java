...
public abstract class ListDiffEntry<E> {
	...
	public static <E> ListDiffEntry<E>[] newArray(int size) {
		return new ListDiffEntry[size];
	}

	public static <E> ListDiffEntry<E>[] newArrayFrom(
			Collection<ListDiffEntry<E>> entryList) {
		ListDiffEntry<E>[] entryArray = newArray(entryList.size());
		int i = 0;
		for (ListDiffEntry<E> entry : entryList)
			entryArray[i++] = entry;
		return entryArray;
	}
